...
const MediaFiltersContainer = (props: Partial<MediaFiltersContainerProps>) => {
  ...
  }: Partial<MediaFiltersContainerProps> = props;
      ...
      <AnimatePresence>
        {menuEnabled &&
          <MotionConfig transition={{ duration: 0.5, bounce: 0.1, type: 'spring' }}>
            <motion.div className='fixed inset-0 w-screen h-screen flex justify-center bg-black/80 overflow-y-auto z-30'
              layout
              initial={{ y: '100%', opacity: 0 }}
              animate={{ y: 0, opacity: 1}}
              exit={{ y: '100%', opacity: 0}}>
              <motion.div className='bg-[rgb(18,18,18)] min-w-40 max-w-[700px] w-full z-31 relative
